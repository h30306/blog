...
summary: "LeetCode Problem Solving - DP on jagged rows / in-place bottom-up accumulation"
...
I update the triangle in place so that each entry becomes the minimum path sum to reach that position. The left edge has only one parent directly above, the right edge has only one parent aboveleft, and middle cells can
